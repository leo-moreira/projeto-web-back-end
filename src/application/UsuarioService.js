// src/application/UserAppService.js
...
const { Usuario } = require('../dominio/usuarios/usuarios');
...
class UsuarioService {
    ...
    async registrarUsuario(userData) {
        ...
        try {
            ...
            const existingUserByEmail = await this.userRepository.procurarPorEmail(usuario.email);
            if (existingUserByEmail) {
                Logger.warn(`Tentativa de registro com email já existente: ${usuario.email}`);
            ...
            Logger.error(`Erro no serviço ao registrar usuário: ${error.message}`, error);
            throw error;
        }
    }
    ...
    async procurarUsuarioPorId(userId) {
        try {
            ...
            Logger.error(`Erro no serviço ao buscar usuário por ID: ${userId} - ${error.message}`, error);
            throw error;
        }
    }
    ...
    async procurarUsuarioPorEmail(email) {
        try {
            ...
            Logger.error(`Erro no serviço ao buscar usuário por email: ${email} - ${error.message}`, error);
            throw error;
        }
    }
    ...
    async procurarTodosOsUsuarios() {
        try {
            ...
            Logger.error(`Erro no serviço ao buscar todos os usuários: ${error.message}`, error);
            throw error;
        }
    }
    ...
    async atualizarUsuario(userId, updateData) {
        try {
            const updatedUser = await this.userRepository.atualizar(userId, updateData);
            if (!updatedUser) {
                Logger.info(`Usuário não encontrado para atualização no serviço: ${userId}`);
                return null;
            }
            ...
            Logger.error(`Erro no serviço ao atualizar usuário ${userId}: ${error.message}`, { originalError: error, updateData });
            throw error;
        }
    }
    ...
    async deletarUsuario(userId) {
        try {
            ...
            if (success) {
                ...
                Logger.info(`Usuário não encontrado para deleção no serviço: ${userId}`);
            }
            return success;
        } catch (error) {
            Logger.error(`Erro no serviço ao deletar usuário ${userId}: ${error.message}`, error);
            throw error;
        }
    }
}
